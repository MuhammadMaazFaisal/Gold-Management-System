...
<head>
    ...
    <title>Users | XML Workflow </title>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php
                                    $sql = "SELECT u.AddedBy AS added_by, u.UserStatus As status, u.UserName AS username, u.UserID AS user_id, u.UserEmail AS email, 
                    GROUP_CONCAT( DISTINCT a.ProcessingStageID SEPARATOR ',' ) AS activity 
                    FROM Users AS u LEFT JOIN UserProcessingStages AS a ON u.UserID = a.UserID GROUP BY u.UserID
                    ";
                                    $stmt = $pdo->prepare($sql);
                                    if ($stmt->execute()) {
                                        ...
                                        <table id="datatable" class="display table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;" role="grid">
                                            ...
                                            <tbody>
                                                <tr> <?php while ($row = $stmt->fetch()) { ?>
                                                        ...
                                                        <td><a class="btn btn-info btn-sm" href="user_processing_stage_edit.php?id=<?php echo $row["user_id"]; ?>">Modify</a></td>
                                                </tr>
                                            <?php } ?>
