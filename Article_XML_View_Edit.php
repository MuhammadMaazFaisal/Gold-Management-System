          ...
          <div class="row">
            <div class="col-12">
              <div class="page-title-box d-flex align-items-center justify-content-space-between">
                ...
                ?>
                <h4 class="page-title mb-0 font-size-18">Article XML Source File Display And Edit</h4>
            ...
            <div class="row">
                ...
                <div class="card">
                  <div class="card-body">
                    <?php  if(isset($_GET['save'])){
                      $data = htmlentities($_GET['code']);
                      $path = $_GET['hiddenFilePath'];
                      $file = fopen($path,"w");
                      if(fwrite($file,$data)){
                        echo "<script language='javascript'>
                        document.getElementById('fileAlert').innerHTML = '<strong>File Save Successfully<strong>';
                        document.getElementById('fileAlert').style.display = 'block';
                        document.getElementById('fileAlert').style.fontWeight = '700';
                        </script>
                        ";
                        fclose($file);
                      }
                    ...
                    <form>
                      ...
                      <?php
                        // if(isset($_GET['from']) && $_GET['from'] == 'edit'){
                      ?>

                      <div class="mt-4">
                        <a type="submit" class="btn btn-primary w-md waves-effect">HTML Preview</a>
                        <a href = "Discover_figures.php?file_id=<?php echo $fileId;?>" type="submit" class="btn btn-secondary waves-effect" >Discover Figures</a>
                        <a  onclick="removeSpaceFromXMLTag()" class="btn btn-secondary waves-effect">XML Cleaner</a><!-- remove garbage values -->
                        <a type="submit" class="btn btn-secondary waves-effect disabled-link">XML Validate</a><!-- just dtd validate with check box versions -->
                        
                        
                        <a type="submit" class="btn btn-secondary waves-effect" >Quality Check</a><!-- Jatz Article Check -->
                        <button class="btn btn-secondary waves-effect" id = "save-file" name ="save">Save</button>
                        <input type="button" id="btnRemove" class="btn btn-secondary waves-effect" value="Remove Whitespace" />
                        
                        <button type="submit" class="btn btn-primary waves-effect" >Complete</button>
                        
                      </div>
                        
                        <!-- <button onclick="click()" class="btn btn-secondarymt-3">Find Replace</button> -->
                      <?php //}?>
                  ...
                  <div class="card-body">
                    <div class="page-title-box align-items-center justify-content-between">
                      <h4 class="page-title mb-0 font-size-18">Check List</h4>
                        <div id = "fileAlert"class="mt-3 alert alert-success alert-border-leftfade show" style = "display:none" role="alert">
              ...
              <?php 
                if(isset($_GET['save'])){
                  echo "<script language='javascript'>
                  document.getElementById('fileAlert').innerHTML = 'File Save Successfully';
                  document.getElementById('fileAlert').style.display = 'block';
      ...
      <script type="text/javascript">
        $(function () {
            $("#btnRemove").click(function () {
                var txtName = editor.getValue();
                var whitespace = txtName.replace(/\n/g, "").trim();
                var x = whitespace.replace(/\>\n[\t ]+\</g, "<").trim();
                var y = x.replace(/\>[\t ]+\</g, "><").trim();
                var z = y.replace(/\>[\t ]+$/g, ">").trim();
